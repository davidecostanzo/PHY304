...
Which can be used to calculate the uncertainty on the particle energy (and hence its mass). The uncertainty $\Delta E$ corresponds to the half-width of the particle's mass distribution. The **full width** of a particle, which is indicated by the letter $\Gamma$, is defined as $\Gamma = 2 \Delta E$. This allows the uncertainty relation to be expressed as:

$$
\Gamma \Delta t \geq \hbar
$$

We saw how the measurement of the width $\Gamma$ of the $Z^{0}$ boson can be used to constrain the number of neutrino families.
...
Isospin can have values that are either integers or half-integer and follows the same algebra as the spin of a particle. Due to the commutation relations of the angular momentum operators, we can only simultaneously define the magnitude of the isospin ($I^2$) and one of its components. By convention we choose the component 3 as the independent component that can be defined and we call that $I_{3}$.
...
In this case, since there is no colour part to the total wavefunction, the product of spin and isospin states must be antisymmetric. We will denote $s_{2}=1 / 2$ by $\uparrow$ and $s_{2}=-1 / 2$ by $\downarrow$, and $I_{3}=1 / 2$ by p (proton) and $I_{3}=-1 / 2$ by n (neutron).
...
- uuu is $\left(I, I_{3}\right)=(3 / 2,3 / 2)$ so must be the $\Delta^{++}$.
- uud is $I_{3}=1 / 2$; forming combinations which are symmetric under 2-quark permutations we get $1 / \sqrt{3}(u u d+u d u+d u u)$. This must be $\left(I, I_{3}\right)=(3 / 2,1 / 2)$ so the $\Delta^{+}$.

- Similarly $1 / \sqrt{3}(d d u+u d d+ d u d)$ is $\left(I, I_{3}\right)=(3 / 2,-1 / 2)$ so the $\Delta^{0}$.
...
The formula for the electric charge needs to be modified to include the strangeness quantum number
...
Families of particles with similar properties (e.g. same spin and parity) can be plotted in terms of $Y$ versus $I_{3}$, and form regular geometrical patterns (see {numref}`Barions` and {numref}`Mesons` and {numref}`multiplets`):
...
This table shows a selection of strange and non-strange baryon and meson multiplets, detailing their quantum numbers: Baryon Number ($B$), Strangeness ($S$), Isospin ($I$), mean Electric Charge ($\langle Q \rangle / e$), and Hypercharge ($Y=B+S$).


```{table} A selection of strange and non-strange baryon and meson multiplets.
:name: multiplets
| Isospin Multiplet | Particles | $B$ | $S$ | $I$ | $\langle Q \rangle / e$ | $Y = B+S$ |
| :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| $\pi^{+}, \pi^{0}, \pi^{-}$ | $\pi$ Mesons | 0 | 0 | 1 | 0 | 0 |
| $p, n$ | Nucleons | 1 | 0 | $1/2$ | $1/2$ | 1 |
| $\Delta^{++}, \Delta^{+}, \Delta^{0}, \Delta^{-}$ | $\Delta$ Baryons | 1 | 0 | $3/2$ | $1/2$ | 1 |
| $\Lambda$ | Lambda Hyperon | 1 | -1 | 0 | 0 | 0 |
| $\Sigma^{+}, \Sigma^{0}, \Sigma^{-}$ | Sigma Hyperons | 1 | -1 | 1 | 0 | 0 |
| $K^{+}, K^{0}$ | Kaon Mesons | 0 | 1 | $1/2$ | $1/2$ | 1 |
| $\overline{K^{0}}, K^{-}$ | Anti-Kaon Mesons | 0 | -1 | $1/2$ | $-1/2$ | -1 |
| $\Xi^{0}, \Xi^{-}$ | Xi Hyperons | 1 | -2 | $1/2$ | $-1/2$ | -1 |
| $\Omega^{-}$ | Omega Hyperon | 1 | -3 | 0 | -1 | -2 |
```
...
:width: 600px
:align: center
The lowest mass baryon octet with spin parity $J^P=\frac{1}{2}^+$ (Left).  The lowest mass baryon decuplet with spin-parity $J^P=\frac{3}{2}^+$ (Right).
```
...
:width: 600px
:align: center
The lowest mass meson states with spin-parity $J^P=0^-$ (Left) and $J^P=1^-$ (Right).
```
...
```{table} Quark quantum numbers and masses. Note the convention that quarks with a negative electric charge carry a negative flavour quantum number.
:name: tableQuark
