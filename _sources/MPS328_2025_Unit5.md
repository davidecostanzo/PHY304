...
In this unit we continue our study of the fundamental interactions of particle physics. We reuse the knowledge we acquired to describe QED to study the strong interactions between quarks and gluons. Quark and gluons carry a "colour" that can be seen as an equivalent of the electric charge in elecrodynamics. The interaction of particles with coulour goes under the name of Quantum ChromoDynamics or QCD in brief. 
...
- It binds quarks in only two configurations: $q q q$ for baryons (and $\bar{q} \bar{q} \bar{q}$ for antibaryons) and $q \bar{q}$ for mesons. 

```{note} The discovery of states known as "pentaquarks" was recently announced. These states are made of 4 quarks and one anti-quark, and they are very unstable. We will not consider these here, but for completeness we should point out their existence
```
...
The strong interaction is a gauge interaction mediated by a massless, spin 1 gluon, $g$, which is electrically neutral but carries both a colour and an anti-colour such as red- antiblue. The coupling constant is known as $\alpha_{S}$ (alpha-strong) and the theory is known as **Quantum Chromodynamics** or QCD in analogy with QED. {numref}`QCDFeynman`(a) shows the interaction between two quarks of different colour (red and blue in this example) by exchanging a gluon carrying red-antiblue. Unlike in QED, the gluon e.g. the exchange quantum is also a source of the field as it carries colour, so processes such as the branching of one gluon into two can occur, with the vertex in {numref}`QCDFeynman`(b) also possible along with the vertex in {numref}`QCDFeynman`(c) which is similar to the basic QED vertex. 

```{figure} QCDFeynman.png
:alt: Feynman diagrams for QCD
:name: QCDFeynman
:width: 600px
:align: center
Example Feynman diagrams for the QCD interactions.
```
A consequence of the additional gluon interaction vertex is a different expression for the static QCD potential as a function of the distance to the source $(r)$, which takes the form:

$$
\begin{equation*}
U_{S}=\frac{4}{3} \frac{\alpha_{S} \hbar c}{r}+k r
\end{equation*}
$$
...
$$
\begin{equation*}
U_{e m}=\frac{1}{4 \pi \epsilon_{0}} \frac{e^{2}}{r}=\frac{\alpha_{e m} \hbar c}{r} 
\end{equation*}
$$
...
The behaviour of a quark-antiquark pair is shown in {numref}`qqbarstring`  (which is not a Feynman diagram, but just a drawing of how a quark-antiquark pair behaves). As the distance increases from picture (1) to picture (2), the potential energy increases. If we keep pulling, there is enough potential energy to create a new quark-antiquark pair as in picture (3). Hence quarks cannot be isolated and studied individually, unlike electromagnetic charged particles. This property is called quark confinement,
and is a specific feature of QCD derived by the presence of additional interaction vertexes as the one in {numref}`QCDFeynman`(b).

```{figure} qqbarstring.png
:alt: Feynman diagrams for QCD
:name: qqbarstring
:width: 600px
:align: center
Picture of a $q \bar{q}$ pair being pulled apart until a new pair is created..
```
...
```{admonition} Example 5.1. Interaction strength of the strong force. 
...
is described by the Feynman diagram in {numref}`eemumu` (left). A similar interaction can produce a quark-antiquark pair as in {numref}`eemumu` (right). The two diagrams are  identical, the only difference is that the charge of the particles on the right side of the Feynman diagrams.

```{figure} eemumu.png
:alt: Feynman diagrams for the mumu production process
:name: eemumu
:width: 600px
:align: center
Feynman diagrams for the QED $e^{+} e^{-} \rightarrow \mu^{+} \mu^{-}$process (left) and the QED $e^{+} e^{-} \rightarrow q \bar{q}$ process (right)
```

In the case of the muon pair production the amplitude of the interaction is proportional to the product of the charges of the electron-positron pair (left) and that of the muon-antimuon pair (right), e.g. $e^{2}$, and the cross section is proportional to the square of the amplitude,
...
$$
\begin{equation*}
R=\frac{\sigma\left(e^{+} e^{-} \rightarrow \text { hadrons }\right)}{\sigma\left(e^{+} e^{-} \rightarrow \mu^{+} \mu^{-}\right)}=\sum_{i} z_{i}^{2} 
\end{equation*}
$$
...
Note that since each quark can exist in 3 colours, the sum must be over both colours and flavours. 

```{admonition} Example 5.2. Calculate the ratio $R$ as a function of the centre-of-mass energy
Evaluate the value of $R$ for the centre of mass energy $E<3$ GeV, $3\ \mathrm{GeV}<E<10\ \mathrm{GeV}$ and $E>10 \mathrm{GeV}$.
```

```{dropdown} Solution


- At low energies, where $u, d$ and $s$ quarks can be produced, but below the threshold for the production of a $c \bar{c}$ pair  $E<3 \mathrm{GeV}$ the value of $R$ is then predicted to be
...
- Once $c \bar{c}$ pair can be produced, but  $b \bar{b}$ pairs  are not accessible, e.g. $3\ \mathrm{GeV}<E<10\ \mathrm{GeV}$ we have

$$
\begin{equation*}
R=3\left[\left(\frac{2}{3}\right)^{2}+\left(\frac{1}{3}\right)^{2}+\left(\frac{1}{3}\right)^{2} + \left(\frac{2}{3}\right)^{2}\right]=\frac{10}{3}
\end{equation*}
$$

- When also $b \bar{b}$ pairs can be produced we have 

$$ 
\begin{equation*}
R=3\left[\left(\frac{2}{3}\right)^{2}+\left(\frac{1}{3}\right)^{2}+\left(\frac{1}{3}\right)^{2} + \left(\frac{2}{3}\right)^{2} +\left(\frac{1}{3}\right)^{2}  \right]=\frac{11}{3}
\end{equation*}
$$ 
...
##  Hadron jets and the discovery of the gluon
...
Similarly, gluons can also be emitted as a result of a scattering and since they also carry colour they are observed as jets of particles. This is shown as a diagram in {numref}`jets`

```{figure} jets.png
:alt: quark and gluon hadronisation into jets
:name: jets
:width: 600px
:align: center
Diagram showing the hadronisation of quarks and gluons into jets. Illustration of the $e^{+} e^{-} \rightarrow \bar{q} q$ (left) and the $e^{+} e^{-} \rightarrow \bar{q} q g$ interaction (right). These are not Feynman diagrams
```

```{admonition} Example 5.3. Feynman diagrams for quarks and gluons production
...
````{dropdown} Solution
The Feynman diagram is the same as the one in {numref}`eemumu` (right),  where we added a gluon to one of the quark legs in the final state

```{figure} threejetgluon.png
:alt: Feynman diagrams for ee to qqg production
:name: threejetgluon
:width: 400px
:align: center
 Feynman diagram for the production of two quarks and a gluon in electron positron collisions.
```

Since there are two electromagnetic vertexes and one QCD vertex, the cross section will be proportional to $\alpha_{em}^2 \alpha_S$

````

Some of the initial results on hadron jets are presented in [Perkins], and refer to the study of $e^{+} e^{-} \rightarrow \bar{q} q$ at a centre of mass energy of 30 GeV ({numref}`threejets` left, from Perkins Fig 6.8a). In a fraction of the collisions events with three jets were observed compatible with the observation of the $e^{+} e^{-} \rightarrow \bar{q} q g$ interaction and the emission of a gluon in the final state ({numref}`threejets` right, see Perkins Fig 6.8b). The experimental results show the particles emerging from the vertex on a plane perpendicular to the direction of travel of the electron-positron pair. In the events shown, two or three clear clusters of hadron jets are visible. The observation of the angular correlations 
...
```{figure} threejets.png
:alt: quark and gluon hadronisation into jets
:name: threejets
:width: 600px
:align: center
Experimental results for the $e^{+} e^{-} \rightarrow \bar{q} q$ interaction (left) and the $e^{+} e^{-} \rightarrow \bar{q} q g$ (right). The lines represent particles emerging from the interaction in a cross-section view of an experimental detector
```
...
From an experimental point of view, the deep inelastic scattering is an extension of the elastic scattering we studied in Unit 2 to measure the charge distribution of nuclei. The main difference is that the energy of the electrons, used as probes, is greatly increased to 10 GeV or more. Because of that, we need to introduce a new notation which considers the fact that the fact that the nucleons break up and absorb some of the initial energy (hence the process is inelastic) and that the kinematics of the particles involved need to be treated using special relativity.
 ...
 {numref}`DIS`  shows a drawing of a deep inelastic scattering collision between an electron and a proton, with the proton breaking up into a hadron system. We indicate with $W$ the invariant mass of the hadron system calculated using the Lorentz
invariant expression

$$
\begin{equation*}
W^2=\left(\sum_{j} E_{j}\right)^{2}-\left(\sum_{j} \vec{p}_{j}\right)^{2} 
\end{equation*}
$$
...
```{figure} DIS.png
:alt: Diagram of deep inelastic scattering
:name: DIS
:width: 300px
:align: center
Diagram of a deep inelastic scattering interaction between an electron and a proton.
```
...
$$
\begin{equation*}
Q^{2}=-\left[\left(E_{i}-E_{f}\right)^{2}-\left(\vec{p}_{i}-\vec{p}_{f}\right)^{2}\right]
\end{equation*}
$$
...
$$
\begin{equation*}
W=M \quad \text { hence } \quad Q^{2}=2 M \nu 
\end{equation*}
$$
...
However, unlike the nucleon, the quark will not be at rest, having considerable momentum within the proton. When we perform a Lorentz transform from the rest frame of a quark to that of the proton, integrating over the distribution of quark momenta leads to the form factor of the proton. However, as long as the quarks are point-like the form factor should only depend on $Q^{2}$ through the dimensionless ratio $x$, and the cross section shows "scale invariance". This is indeed observed, as shown in {numref}`DISscaling`. (Here $F_{2}$ is proportional to the form factor).

```{figure} DISscaling.png
:alt: deep inelastic scattering scaling
:name: DISscaling
:width: 600px
:align: center
$F_{2}$ as a function of $q^{2}$ at $x=0.25$. For this choice of $x$, there is practically no dependence on $Q^{2}$, that is, there is exact "scale invariance". (Data from the Stanford Linear Accelerator Center.).
```
...
- Nobel prize for the deep inelastic scattering https://www.nobelprize.org/prizes/physics/1990/9589-the-deep-inelastic-collision/
- Normalisation and 1999 Nobel prize https://www.nobelprize.org/prizes/physics/1999/press-release/


## Addendum: The colour of gluons
...
$$
\begin{equation*}
G_{0}=\frac{1}{\sqrt{3}}(r \bar{r}+g \bar{g}+b \bar{b} )
\end{equation*}
$$
...
$$
\begin{align*}
G_{7} & =\frac{1}{\sqrt{2}}(r \bar{r}-b \bar{b})  \\
G_{8} & =\frac{1}{\sqrt{6}}(r \bar{r}-2 g \bar{g}+b \bar{b}) 
\end{align*}
$$
