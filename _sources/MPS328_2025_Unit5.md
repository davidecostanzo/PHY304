...
is described by the Feynman diagram in {numref}`eemumu` (left). A similar interaction can produce a quark-antiquark pair as in {numref}`eemumu`. The two diagrams are  identical, the only difference is that the charge of the particles on the left side of the Feynman diagrams.
...
```{admonition} Example 5.2. The ratio $R$ as a function of the centre-of-mass energy
